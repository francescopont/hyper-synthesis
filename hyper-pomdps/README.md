...
* the original POMDP in PRISM syntax (`/sketch.templ`);
...
* a json file with some necessary information to build a 
  ...
  * a map from states of the cross-product to observation tuples of the individual agents, 
...
* the corresponding Dec-POMDP in Cassandra format (which can be used as input for Inf-JESP) 
